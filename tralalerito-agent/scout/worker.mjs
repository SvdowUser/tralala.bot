...
async function main() {
  ...
  const existingDraftIds = new Set((drafts.items || []).map(x => x.sourceMissionId));

  const newDrafts = filteredMissions
    .map((mission) => {
      const id = slugify(mission.title);
      return {
        id,
        ...
        draftText: mission.examplePost || "",
        tone: mission.introStyle || "cute, curious, baby-like",
        targetAngle: mission.liveWhyNow || mission.whyThisFits || "",
        riskCheck: mission.riskNote || "",
        liveWhyNow: mission.liveWhyNow || "",
        sourceUrls: Array.isArray(mission.sourceUrls) ? mission.sourceUrls : []
      };
    })
    .filter((draft) => draft.sourceMissionId && !existingDraftIds.has(draft.sourceMissionId));

  drafts.items = [...(drafts.items || []), ...newDrafts];
  ...
  console.log(JSON.stringify({ items: newDrafts }, null, 2));
